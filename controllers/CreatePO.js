...
router.get("/next-po-number", async (req, res) => {
  try {
    const latestPO = await po_processing_assignment_staging.findOne({
      ...
      where: {
        po_num: {
          [require("sequelize").Op.like]: "PO # IT Equip:%",
        },
      },
    });
    ...
    if (latestPO && latestPO.po_num) {
      const match = latestPO.po_num.match(/PO # IT Equip:\s*(\d+)/);
      if (match && match[1]) {
        nextNumber = parseInt(match[1]) + 1;
      }
    }

    const nextPONum = `PO # IT Equip: ${nextNumber.toString().padStart(4, "0")}`;
...
//         const gst = poData.gst;
//         const cgst = gst / 2;
//         const sgst = gst / 2;

//       await po_products_staging.create({
//         product_id: `${po_num}-product-${i + 1}`, // 🛠 unique ID, remove spaces
//         po_num: po_num,
...
//         total_price_incl_gst: parseFloat((item.quantity * item.unit_price * (1 + gst / 100)).toFixed(2)),
//       });
//     }

//     // ✅ 3. Continue rest of logic
...
//       }
//       poUrl = await uploadToS3(pdfFilePath, po_num);
...
//     // ✅ 5. Send email
//     // ✅ Fetch approver details (similar to maintenance API)
