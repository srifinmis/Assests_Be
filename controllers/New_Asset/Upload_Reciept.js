...
router.post("/upload_receipt", upload.single("paymentFile"), async (req, res) => {
  ...
  try {
    ...
    if (!po_number || !utr_number || !payment_date || !base_location || !state || !assetData || !file || !asset_type) {
    ...
    // ✅ Create payment assignment staging entry
    ...
    await transaction.rollback();
    res.status(500).json({ error: "Failed to upload payment and send approval request." });
  }
});
